...
export default function Dashboard() {
  ...
  const statusColors: Record<string, string> = {
    Novo: 'bg-white/20 text-white border border-white/30',
    Preparando: 'bg-white/20 text-white border border-white/30',
    Pronto: 'bg-white/20 text-white border border-white/30',
    Entregue: 'bg-white/20 text-white border border-white/30',
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <div ref={userButtonRef}>
            <button
              ...
              className="w-9 h-9 rounded-xl bg-white/20 flex items-center justify-center hover:bg-white/30 transition-all"
            >
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6 space-y-6 pb-24">
        ...
        <section className="grid grid-cols-2 gap-3">
          <div className="bg-gradient-to-br from-cyan-400 to-cyan-300 rounded-2xl p-4 flex flex-col justify-between min-h-[100px]">
            <div className="flex items-center gap-2 text-white/80">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider text-white">Pedidos</span>
            </div>
          ...
          <div className="bg-gradient-to-br from-sky-400 to-sky-300 rounded-2xl p-4 flex flex-col justify-between min-h-[100px]">
            <div className="flex items-center gap-2 text-white/80">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider text-white">Faturamento</span>
            </div>
        ...
        {!carregando && stats.totalPedidos > 0 && (
          <section className="space-y-3">
            <h2 className="text-sm font-semibold text-white uppercase tracking-wider">Pedidos por Status</h2>
            <div className="grid grid-cols-4 gap-2">
              {[
                { label: 'Novos', count: stats.pedidosNovos, color: 'text-white', bg: 'bg-cyan-400/20', icon: Timer },
                { label: 'Preparo', count: stats.pedidosPreparando, color: 'text-white', bg: 'bg-sky-400/20', icon: AlertTriangle },
                { label: 'Prontos', count: stats.pedidosProntos, color: 'text-white', bg: 'bg-blue-400/20', icon: CheckCircle2 },
                { label: 'Entregues', count: stats.pedidosEntregues, color: 'text-white', bg: 'bg-gray-400/20', icon: Package },
              ].map((item) => (
                <div key={item.label} className={`${item.bg} rounded-xl p-3 text-center`}>
                  ...
                  <span className="text-[8px] font-bold uppercase tracking-wider text-white/70">{item.label}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        {!carregando && (stats.mesasOcupadas + stats.mesasLivres) > 0 && (
          <section className="space-y-3">
            <h2 className="text-sm font-semibold text-white uppercase tracking-wider">Mesas</h2>
            <div className="flex gap-3">
              <div className="flex-1 bg-cyan-400/20 rounded-xl p-3 flex items-center gap-3">
                ...
                <div>
                  <div className="text-lg font-bold font-mono text-white">{stats.mesasOcupadas}</div>
                  <span className="text-[9px] font-bold uppercase tracking-wider text-white/70">Ocupadas</span>
                </div>
              </div>
              <div className="flex-1 bg-sky-400/20 rounded-xl p-3 flex items-center gap-3">
                ...
                <div>
                  <div className="text-lg font-bold font-mono text-white">{stats.mesasLivres}</div>
                  <span className="text-[9px] font-bold uppercase tracking-wider text-white/70">Livres</span>
                </div>
              </div>
            </div>
          </section>
        ...
        <section className="space-y-4">
          <h2 className="text-sm font-semibold text-white uppercase tracking-wider">Ações Rápidas</h2>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => navigate('/pdv')}
              className="bg-cyan-400/80 backdrop-blur-md border border-white/20 text-white rounded-2xl p-4 flex flex-col items-start gap-3 min-h-[110px] hover:bg-cyan-400 active:brightness-90 transition-all"
            >
              <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center">
                <PlusCircle size={22} className="text-white" />
              </div>
              <span className="text-sm font-bold text-left mt-auto text-white">Novo Pedido</span>
            </button>
            <button
            ...
            >
              ...
              <span className="text-sm font-semibold text-left mt-auto text-white">Mapa de Mesas</span>
            </button>
            <button
              onClick={() => navigate('/sala')}
              className="bg-cyan-400/80 backdrop-blur-md border border-white/20 text-white rounded-2xl p-4 flex flex-col items-start gap-3 min-h-[110px] hover:bg-cyan-400 active:brightness-90 transition-all"
            >
              ...
              <span className="text-sm font-semibold text-left mt-auto text-white">Fechar Conta</span>
            </button>
            <button
            ...
            >
              ...
              <span className="text-sm font-semibold text-left mt-auto text-white">Delivery</span>
            </button>
          </div>
        </section>

        {stats.ativoRecente.length > 0 && (
          <section className="space-y-3 border-t border-white/20 pt-6">
            <h2 className="text-sm font-semibold text-white uppercase tracking-wider">Atividade Recente</h2>
            <div className="flex flex-col bg-white/10 border border-white/20 rounded-2xl overflow-hidden">
